...
func GetIpArray() (ips []string, err error) {
	...
	if err != nil {
		err = errors.Wrap(err, `net.InterfaceAddrs failed`)
		return nil, err
	}
...
func GetIntranetIps() (ip string, err error) {
	ips, err := GetIntranetIpArray()
	if err != nil {
		return "", err
	}
...
func GetIntranetIpArray() (ips []string, err error) {
	...
	for _, interFace := range interFaces {
		...
		if err != nil {
			err = errors.Wrap(err, `interFace.Addrs failed`)
			return ips, err
		}
...
func IsIntranet(ip string) bool {
	...
	if array[0] == "172" {
		...
		if err != nil {
			err = errors.Wrapf(err, `strconv.ParseInt failed for string "%s"`, array[1])
			return false
		}
