...
import (
	...
	"github.com/xwc1125/xwc1125-pkg/middleware/circuitbreaker/sre/window"
...
func (l *BBR) Allow() bool {
	if l.shouldDrop() {
		l.log.Warn(limiter.ErrLimitExceed.Error())
		return false
	}
...
func (l *BBR) SetWindow(window time.Duration) {
	l.lock.Lock()
	defer l.lock.Unlock()
	bucketDuration := window / time.Duration(l.bucket)
	passStat := window.NewRollingCounter(window.RollingCounterOpts{Size: l.bucket, BucketDuration: bucketDuration})
	rtStat := window.NewRollingCounter(window.RollingCounterOpts{Size: l.bucket, BucketDuration: bucketDuration})
	l.window = window
	l.passStat = passStat
	l.rtStat = rtStat
	l.bucketDuration = bucketDuration
	l.bucketPerSecond = int64(time.Second / bucketDuration)
}

func (l *BBR) SetBurst(burst int) {
	l.lock.Lock()
	defer l.lock.Unlock()
	bucketDuration := l.window / time.Duration(burst)
	passStat := window.NewRollingCounter(window.RollingCounterOpts{Size: burst, BucketDuration: bucketDuration})
	rtStat := window.NewRollingCounter(window.RollingCounterOpts{Size: burst, BucketDuration: bucketDuration})
	l.bucket = burst
	l.passStat = passStat
	l.rtStat = rtStat
	l.bucketDuration = bucketDuration
